...
switch ($SESSION["id"]) {
	case 1:
		...
		if ($SESSION["show_thumbnail"] == 1) {
			...
			if ($SESSION["back"] == 1) {
				echo "<div class=\"folder\" onmouseout=\"this.style.backgroundColor='#ffcc66'\" onmouseover=\"this.style.backgroundColor='#ff9900'\"><div class=\"image\" onclick=\"BackFolder('".bin2hex(RC4("id=1&".$GET))."');\"><img style=\"margin-top:".($SESSION["thumbnail_size"] / 2 - 16)."px;\" src=\"img/icon_folder_back_32x32.gif\" border=\"0\" /></div><div class=\"textbox\">&nbsp;</div></div>";
			}
			...
			for ($j = 0; $j < count($FOLDERS); $j++) {
				echo "<div class=\"folder\" onmouseout=\"this.style.backgroundColor='#ffcc66'\" onmouseover=\"this.style.backgroundColor='#ff9900'\"><div class=\"image\" onclick=\"OpenFolder('".bin2hex(RC4("id=1&dir=".$IMAGE_PATH_2.$FOLDERS[$j]."/&page=0&back=1&".$GET2))."');\"><img style=\"margin-top:".($SESSION["thumbnail_size"] / 2 - 16)."px;\" src=\"img/icon_folder_32x32.gif\" border=\"0\" /></div><div class=\"textbox\" title=\"".$FOLDERS[$j]."\" onmouseover=\"ShowImageMenu('imenu".$IMENU_ID."', 'block');\" onmouseout=\"ShowImageMenu('imenu".$IMENU_ID."', 'none');\">".$FOLDERS[$j]."</div>";
			...
			for ($j = $SESSION["page"] * $SESSION["thumbnails_perpage"]; $j < count($FILES); $j++) {
				...
				if (!is_dir($IMAGE_PATH.$FILES[$j])) {
					...
					echo "<div id=\"thumbnail".$j."\" class=\"thumbnail\" onmouseout=\"this.style.backgroundColor='#a8adb4'\" onmouseover=\"this.style.backgroundColor='#1c3b51'\"><span class=\"tip\" onmouseover=\"tooltip('".$image_title."');\" onmouseout=\"exit();\"><div class=\"image\" onclick=\"InsertImage('".$SESSION["server"]."', '".$IMAGE_PATH_2.$FILES[$j]."', '".$image_width."', '".$image_height."');\"><img id=\"th".$j."\" style=\"margin-top:".round(($SESSION["thumbnail_size"] / 2) - ($a["height"] / 2))."px;\" src=\"img/wait_2.gif\" border=\"0\" /></div></span><div class=\"textbox\" title=\"".$FILES[$j]."\" onmouseover=\"ShowImageMenu('imenu".$IMENU_ID."', 'block');\" onmouseout=\"ShowImageMenu('imenu".$IMENU_ID."', 'none');\">".str_replace (' ', '&nbsp;', $FILES[$j])."</div>";
		...
		if ($SESSION["show_thumbnail"] == 0) {
			...
			if ($SESSION["back"] == 1) {
				...
				echo "<td style=\"width:18px; text-align:center; cursor:pointer;\" onclick=\"BackFolder('".bin2hex(RC4("id=1&".$GET))."');\"><img src=\"img/icon_folder_back_16x16.png\" border=\"0\" /></td>";
			...
			for ($j = 0; $j < count($FOLDERS); $j++) {
				...
				// Icon "Verzeichnis" 
				echo "<td style=\"width:18px; text-align:center; cursor:pointer;\" onclick=\"OpenFolder('".bin2hex(RC4("id=1&dir=".$IMAGE_PATH_2.$FOLDERS[$j]."/&page=0&back=1&".$GET2))."');\"><img src=\"img/icon_folder_16x16.png\" border=\"0\" /></td>";
				...
				if ($SESSION["show_folder_menu"] == 1) {
					...
					echo "<td style=\"width:16px; text-align:center;\"><img id=\"i2".$i."\" style=\"cursor:pointer;\" src=\"img/icon_delete_16x16.png\" border=\"0\" title=\"\" onclick=\"Folder_Delete('".bin2hex(RC4("id=1&".$GET))."', '".$FOLDERS[$j]."');\" /></td>";
			...
			for ($j = 0; $j < count($FILES); $j++) {
				...
				echo "<td style=\"width:18px; text-align:center;\"><div id=\"smpreview".$j."\" class=\"smpreview\"></div><img style=\"cursor:pointer;\" src=\"img/icon_image_16x16.png\" border=\"0\" onmouseover=\"SMP.Show('smpreview".$j."', '".$IMAGE_PATH_2.$FILES[$j]."', '".$a["width"]."', '".$a["height"]."');\" onmouseout=\"SMP.Close('smpreview".$j."');\" onclick=\"InsertImage('".$SESSION["server"]."', '".$IMAGE_PATH_2.$FILES[$j]."', '".Image_GetWidth($IMAGE_PATH.$FILES[$j])."', '".Image_GetHeight($IMAGE_PATH.$FILES[$j])."');\" /></td>";
				...
				// Icon "Bild einfügen"
				echo "<td style=\"width:16px; text-align:center;\"><img id=\"i1".$i."\" style=\"cursor:pointer;\" src=\"img/icon_insert_16x16.png\" border=\"0\" title=\"\" onclick=\"InsertImage('".$SESSION["server"]."', '".$IMAGE_PATH_2.$FILES[$j]."', '".Image_GetWidth($IMAGE_PATH.$FILES[$j])."', '".Image_GetHeight($IMAGE_PATH.$FILES[$j])."');\" /></td>";

				// Icon "Bilddatei löschen"
				if ($SESSION["show_image_menu"] == 1) {
					echo "<td style=\"width:16px; text-align:center;\"><img id=\"i2".$i."\" style=\"cursor:pointer;\" src=\"img/icon_delete_16x16.png\" border=\"0\" title=\"\" onclick=\"Image_Delete('".bin2hex(RC4("id=1&".$GET))."', '".$FILES[$j]."');\" /></td>";
				}
...
if ($SESSION["id"] == 1 && $SESSION["show_thumbnail"] == 0) { echo "SMTableIni();\n"; }
?>
//-->
</script>
